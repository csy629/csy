<template>
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column min-width="100px" align="center" label="banner标题">
        <template slot-scope="scope">
    ...
    <el-dialog :title="`${dialogType}banner`" :visible.sync="dialogFormVisible" class="bannerDialog">
      <el-form :model="bannerForm"  :rules="rules" ref="ruleForm">
        <el-form-item label="banner标题" prop="bannerName" :label-width="formLabelWidth">
...
<script>
  import { getConsultationBanner, editConsultationBanner, updateConsultationBannnerSort } from '@/api/Consultation'
  ...
  export default {
    ...
    methods: {
      getList() {  //获取数据
        ...
        getConsultationBanner().then(response => {
      ...
      handleChange(index, row) {  //表格内上下架
        ...
        editConsultationBanner(data).then(response => {
          this.getList();
